...
const defaultSatellite = 'Ariane-5 VA-200'

export const DashBoardLineChart = ( {activeSatellite, lineChartData} ) => {

    let satelliteName = activeSatellite === undefined ? 'All satellites' : activeSatellite.name;

    const lineChartDataMaxY = Math.max(...lineChartData.numberOfConjunctions);

    console.log('lineChartDataMaxY ', lineChartDataMaxY )
    ...
    function backGroundGradient(ctx, chartArea) {
      ...
      if (gradient === null || width !== chartWidth || height !== chartHeight) {
        ...
        // gradient.addColorStop(0.3, '#25AEE1  ');
    ...
    // console.log('tooltiptitle', tooltiptitle)
        ...
        bodyLines.forEach((body, i) => {
          ...
          const text = document.createTextNode(satelliteName); //bodyArr[1]
          td.appendChild(span);
          td.appendChild(text);
          tr.appendChild(td);
      ...
      const newBodySecond = numBodySecond >= 1000 ? (numBodySecond/1000).toFixed(1) +'k' : numBodySecond
      
      console.log( 'number of conj', newBodySecond)
      const conjWord = newBodySecond<=1 ? 'conjunction' : 'conjunctions'
      // const numBodySecond = 
      const elementToTh = document.querySelector('div>table>thead>tr>th')
          elementToTh.innerText= newBodySecond+ ' ' + conjWord + ' '+`${tooltiptitle} `
    ...
    labels: lineChartData.days,
    datasets: [
      {
          label: satelliteName,
          data: lineChartData.numberOfConjunctions,
        ...
        const config = {
            ...
            scales: {
                ...
                ticks:{ 
                    ...
                    // if (value <= 500 && value>=1) return value
                    // const newValue = value / 1000 + 'k'
                    // if (newValue.length<=3) return newValue + '   '
                    // if (!Number.isInteger(value)){return ''}
                    return value;
                    },
                    stepSize: lineChartDataMaxY <= 10 ? 1 : null,
                    color: '#7CB3BF',
                    backdropColor: 'red',
                    major: false,
                    drawTicks: false,
    ...
    return(
        ...
        <div className="dashBoardLineChart_wrapper">
            <div className="dashBoardLineChart_headerWrapper">
              Number of conjunctions the last year
            </div>
            <div className="dashBoardLineChart_lineChart_wrapper">
                <Line 
                    data={data} 
